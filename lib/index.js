...
const FLASH_PRICING_START = Date.UTC(2026, 8, 10, 4, 0, 0);
/**
...
function isOfficialPriceTable(table) {
...
function attachCollector(ctx, storage, priceTable, options = {}) {
	...
	ctx.on("session/event", (session, event) => {
		...
		const record = buildRecord(session.id, event.data.turn, event.data.step, event.seq, event.time, source.provider, source.model, usage, readPriceTable());
...
function createTokenCostProjectionDefinition(priceTable) {
	...
	return {
		...
		apply: (state, event) => {
			...
			const breakdown = priceUsage(inputTokens, cacheReadTokens, cacheWriteTokens, outputTokens, source.provider, source.model, event.time, readPriceTable());
...
function usageIdentity(record) {
	if (record.sourceEventSeq !== void 0) return JSON.stringify([
		"seq",
		record.sessionId,
...
var UsageStorage = class {
	...
	add(record) {
		...
		if (this.isDuplicateRecord(record)) return void 0;
...
const CURRENT_RELEASE_VERSION = "4.0.9";
...
function isMissingArgumentContractError(error) {
	return error instanceof TypeError && error.message.includes("undefined") && (error.message.includes("at") || error.message.includes("SessionLogOffset"));
}
...
*/
async function rebuildTokenCostSnapshot(ctx, sessionId, knownInspection) {
	const coldSnapshot = ctx.sessionProjectionCache.coldSnapshot;
	if (coldSnapshot.length >= 3) {
		const inspection = knownInspection ?? await inspectSession(ctx, sessionId);
		coldSnapshot.call(ctx.sessionProjectionCache, inspection.meta, inspection.inheritedEventCount, inspection.events);
		return;
	}
	let legacyResult;
	try {
...
async function migrateMissingTokenCost(ctx) {
	let entries;
	try {
		entries = await ctx.sessionPersistence.list();
	} catch (error) {
	...
	for (const entry of entries) {
		...
		try {
			const cachedSnapshot = ctx.sessionProjectionCache.cachedSnapshot;
			let inspection;
			let cached;
			...
			} else cached = cachedSnapshot.call(ctx.sessionProjectionCache, header);
			if (cached?.values?.tokenCost !== void 0) continue;
...
function apply(ctx) {
	...
	const currentPriceTable = () => priceTable;
	...
	attachCollector(ctx, storage, currentPriceTable, { onPersistedRecord: (record) => {
