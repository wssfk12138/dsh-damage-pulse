								...
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
									...
									children: [
										...
										/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
											...
											children: ["当前版本 v", updateStatus?.currentVersion ?? "4.0.3"]
										}),
										/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
...
`;
		...
		*/
		...
		function createRouteEligibilityLoader(connectionOrSessions, fetcher = fetch) {
			return async (sessionId, signal) => {
				try {
					const [models, response] = await Promise.all([loadSessionModels(connectionOrSessions, sessionId, signal), fetcher("/api/token-monitor/pricing-eligibility", {
						cache: "no-store",
						signal
					})]);
		...
		async function loadSessionModels(connectionOrSessions, sessionId, signal) {
			if ("models" in connectionOrSessions && typeof connectionOrSessions.models === "function") return connectionOrSessions.models({ sessionId }, signal);
			const rpc = connectionOrSessions.rpc;
			if (rpc === void 0) throw new Error("session.models is unavailable");
			return { result: await rpc.call("/api", "session.models", { args: {
				agentId: sessionId,
				request: { sessionId }
			} }, signal) };
		}
		...
		function apply(ctx) {
			const loadRouteEligibility = createRouteEligibilityLoader(ctx.get("connection"));
